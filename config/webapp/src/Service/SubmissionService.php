...
class SubmissionService
{
     ...
     * @param string $sort Either 'time' (newest first) or 'problem' (contest order, then newest first).
     *
    ...
    public function getSubmissionList(
        ...
        if ($sort === 'problem') {
            $queryBuilder
                ->addOrderBy('cp.contest', 'ASC')
                ->addOrderBy('cp.sortOrder', 'ASC')
                ->addOrderBy('cp.shortname', 'ASC')
                ->addOrderBy('cp.problem', 'ASC');
        }
        $queryBuilder
