...
class ScoreboardController extends AbstractApiController
{
    ...
    public function getScoreboardAction(
        ...
        foreach ($scoreboard->getScores() as $teamScore) {
            ...
            $problems = [];
            $teamMatrix = $scoreboard->getMatrix()[$teamScore->team->getTeamid()];
            foreach ($scoreboard->getProblems() as $problemId => $contestProblem) {
                if (!isset($teamMatrix[$problemId])) {
                    continue;
                }
                $matrixItem = $teamMatrix[$problemId];
                $problem = new Problem(
