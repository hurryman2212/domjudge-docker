...
class ContestController extends BaseController
{
    ...
    public function editAction(Request $request, int $contestId): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->normalizeProblemOrder($contest);
            /** @var ContestProblem[] $problems */
    ...
    private function normalizeProblemOrder(Contest $contest): void
    {
        $problems = $contest->getProblems()->toArray();
        // Zero is reserved for a newly added problem without an explicit order.
        // Equal positions keep their form order; only contiguous ranks are saved.
        usort($problems, static fn(ContestProblem $a, ContestProblem $b) =>
            ($a->getSortOrder() ?: PHP_INT_MAX) <=> ($b->getSortOrder() ?: PHP_INT_MAX));
        foreach ($problems as $index => $problem) {
            $problem->setSortOrder($index + 1);
        }
    }
